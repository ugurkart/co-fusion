...
class ImageLogReader : public LogReader {
 public:
  ImageLogReader(std::string colorDirectory, std::string depthDirectory, std::string maskDirectory, unsigned indexWidth = 8,
  ...
  const std::string getMaskPath();

  void setCurrentFrameIndex(unsigned int current_frame);
 ...
 private:
  ...
  unsigned nextBufferIndex = 1; // VOT Style starts from 1
  float rateHz = 24;
