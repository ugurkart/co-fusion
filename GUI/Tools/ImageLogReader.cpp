...
ImageLogReader::ImageLogReader(std::string colorDirectory, std::string depthDirectory, std::string maskDirectory, unsigned indexWidth,
    ...
    // Discard these since we'll provide masks on the fly per frame basis
...
void ImageLogReader::bufferFramesImpl() {
    ...
    for (unsigned i = 1; 0 < 15 && nextBufferIndex < frames.size(); ++i, ++nextBufferIndex)
    {
...
//            std::cout << "Warning -- your depth scale is likely to mismatch. Check ImageLogReader.cpp!" << std::endl;
            ...
            for (int i = 0; i < result.depth.rows; ++i)
            {
                ...
                for (int j = 0; j < result.depth.cols; ++j) ((float*)newDepth.data)[depthIdx++] = 0.001 * pixel[j]; //0.0006f * pixel[j];
            }
        }
        else
        {
...
//    cv::Mat mask = cv::Mat(frames[currentFrame].height, frames[currentFrame].width, CV_8UC1);
    frames[currentFrame] = loadFrameFromDrive(currentFrame);
...
bool ImageLogReader::hasMore()
{
    return currentFrame + 1 < numFrames + 1;
}
...
const std::string ImageLogReader::getMaskPath()
{
    return (maskImagesDir + maskPre);
}

void ImageLogReader::setCurrentFrameIndex(unsigned int current_frame)
{
