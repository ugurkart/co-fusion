...
public:
    ...
    //                    const std::string mask_dir="", int current_index=0,
    //                    const bool bootstrap = false);
    bool processFrame(FrameData& frame, Eigen::Matrix4f& out_pose, const Eigen::Matrix4f* inPose = 0, const float weightMultiplier = 1.f,
                      const std::string mask_dir="", const std::string proj_dir="", const std::string center_file="", const std::string icp_error_file="", const std::string all_icp_error_file="",
                      std::string euler_angles_file="", int current_index=0,
                      const bool bootstrap = false);
   ...
   */
    ...
    inline float medianMat(const cv::Mat& Input, const cv::Mat& mask)
    {
        double min_depth, max_depth;
        min_depth = 0; max_depth = 0;
        cv::Point min_loc, max_loc;
        cv::minMaxLoc(Input, &min_depth, &max_depth, &max_loc, &min_loc, mask);

//        std::cout<<min_depth<<" "<<max_depth<<std::endl;
        if((min_depth == 0 && max_depth == 0) || (std::isnan(min_depth)) || (std::isnan(max_depth)) ) return 0;

        float step = 0.01;
        float range_float = max_depth - min_depth;
        int nVals = (range_float / step);

        if(nVals == 0) return max_depth;

                // COMPUTE HISTOGRAM OF SINGLE CHANNEL MATRIX
        float range[nVals];
        for(auto n=0; n<nVals; n++)
        {
            range[n] = min_depth + n*step;
        }

        float histRange[] = {static_cast<float>(min_depth), static_cast<float>(max_depth)};
        const float* ranges = {histRange};

        cv::Mat hist;
        calcHist(&Input, 1, 0, mask, hist, 1, &nVals, &ranges);
...
//            std::cout<<cdf.at<float>(i)<<std::endl;

        }
        cdf /= cdf.at<float>(nVals-1);

        // COMPUTE MEDIAN
        int medianVal_index = -1;
...
//            std::cout<<cdf.at<float>(i)<<std::endl;
        ...
        if(medianVal_index == -1) return 0;
        return range[medianVal_index];
    }
    ...
    inline void setTargetPose(Eigen::Matrix4f in_pose)
    {
        auto it = models.begin();
        it++;
        (*it)->setCurrPose(in_pose);
    }

    float icp_error_threshold_matlab;
