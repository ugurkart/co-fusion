...
void Model::generateCUDATextureMask(GPUTexture* mask)
{
    GPUSetup& gpu = GPUSetup::getInstance();
    std::vector<DeviceArray2D<unsigned char>>& maskPyr = gpu.mask_tmp;
...
void Model::performTracking(bool frameToFrameRGB, Eigen::Vector3f delta_translation, unsigned int model_index, bool rgbOnly, float icpWeight, bool pyramid, bool fastOdom, bool so3,
                            float maxDepthProcessed, GPUTexture* rgb, int64_t logTimestamp, bool doFillIn)
{
    ...
    Eigen::Vector3f transObject;
    if(model_index !=0)
    {
        transObject = delta_translation;
        lastPose.topRightCorner(3, 1) = delta_translation;
    }
    else
    {
...
void Model::uploadMap(Model::SurfelMap& map)
{
    glFinish();
    ...
    glBindBuffer(GL_COPY_WRITE_BUFFER, vbos[renderSource].dataBuffer);
    glBufferData(GL_COPY_READ_BUFFER, count * Vertex::SIZE, &(map.data->front()), GL_STREAM_DRAW);
    glCopyBufferSubData(GL_COPY_READ_BUFFER, GL_COPY_WRITE_BUFFER, 0, 0, count * Vertex::SIZE);
    glBindBuffer(GL_COPY_READ_BUFFER, 0);
    glBindBuffer(GL_COPY_WRITE_BUFFER, 0);
