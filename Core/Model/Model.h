...
class Model {
 ...
 public:
  ...
  static void generateCUDATextureMask(GPUTexture* mask);

  virtual void initICP(bool doFillIn, bool frameToFrameRGB, float depthCutoff, GPUTexture* rgb);

  // ----- Tracking and fusion

  virtual void performTracking(bool frameToFrameRGB, Eigen::Vector3f center_point, unsigned int model_index, bool rgbOnly, float icpWeight, bool pyramid, bool fastOdom, bool so3,
  ...
  struct SurfelMap
  {
    ...
    std::vector<Eigen::Vector4f> getPointCloudBoundaries(const float& confThres)
    {
        std::vector<Eigen::Vector4f> output;

        // TODO: Change these numbers to min max limit values for better coding practice
        float min_x=1000000, min_y=1000000, min_z=1000000;
        float max_x=-1000000, max_y=-1000000, max_z=-100000;
        ...
        min[0] = min_x; min[1] = min_y; min[2] = min_z; min[3] = 1;
        max[0] = max_x; max[1] = max_y; max[2] = max_z; max[3] = 1;
        output.push_back(min); output.push_back(max);
        return output;
    }
  };

  Eigen::Vector3f init_center_point;
  float last_depth;

  virtual SurfelMap downloadMap();

  void uploadMap(Model::SurfelMap& map);

  std::vector<Eigen::Vector4f> wire_cube;
  ...
  inline void setInitialFrameFlag(bool flag) {initial_frame = flag;}
  inline bool getInitialFrameFlag() {return initial_frame;}
  ...
  inline void setCurrPose(const Eigen::Matrix4f& p)
  {
      pose = p;
  }
