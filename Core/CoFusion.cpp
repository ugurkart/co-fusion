...
{
    feedbackBuffers[FeedbackBuffer::RAW] = new FeedbackBuffer(loadProgramGeomFromFile("vertex_feedback.vert", "vertex_feedback.geom"));  // Used to render raw depth data
...
bool CoFusion::processFrame(FrameData& frame, Eigen::Matrix4f& out_pose, const Eigen::Matrix4f* inPose, const float weightMultiplier,
                            const std::string mask_dir, const std::string proj_dir,
                            const std::string center_file, const std::string icp_error_file, const std::string all_icp_error_file, std::string euler_angles_file,
                            int current_index, const bool bootstrap)
{
    ...
    ss << std::setw(8) << std::setfill('0') << current_index;
    std::string index_string(ss.str());
    std::string mask_img_path = mask_dir + index_string + ".png"; // Hardcode png for the time being
    frame.mask = cv::imread(mask_img_path, cv::IMREAD_GRAYSCALE);
    if (frame.mask.total() != frame.rgb.total()){throw std::invalid_argument("Could not read mask-image file.");}
    if (!frame.mask.isContinuous() || frame.mask.type() != CV_8UC1){throw std::invalid_argument("Incompatible mask image.");}
    ...
    // Use ground-truth segmentation if provided
    textures[GPUTexture::MASK]->texture->Upload(frame.mask.data, GL_LUMINANCE_INTEGER_EXT, GL_UNSIGNED_BYTE);
    //} else
    ...
    else
    {
        ...
        if (bootstrap || !inPose)
        {
            ...
            for (auto model : models)
            {
                if(model_index == 1)
                {
                    ...
                    std::ifstream file(center_file);
                    std::string line;
                    std::getline(file, line);

                    Eigen::Vector4f center_point;
                    double center_x, center_y;
                    int dummy = sscanf(line.c_str(), "%lg %lg", &center_x, &center_y);

                    float depth = medianMat(frame.depth, frame.mask);

                    center_point[0] = ((unsigned int)center_x - intrinsic_values.cx()) * depth * (1.0 / intrinsic_values.fx());
                    center_point[1] = ((unsigned int)center_y - intrinsic_values.cy()) * depth * (1.0 / intrinsic_values.fy());
                    center_point[2] = depth;
                    center_point[3] = 1;

                    Eigen::Matrix4f tmp_extrinsic_matrix = (model)->getPose();

                    tmp_extrinsic_matrix.topRightCorner(3, 1) = Eigen::Vector3f(0,0,0);
                    center_point = tmp_extrinsic_matrix * center_point;
                    center_point = center_point / center_point[3];

                    Eigen::Vector3f delta_translation;

                    if(model->first_frame)
                    {
                        model->init_center_point = Eigen::Vector3f (center_point[0], center_point[1], center_point[2]);
                        delta_translation = Eigen::Vector3f(0, 0, 0);
                        model->first_frame = false;
                    }
                    else
                    {
                        delta_translation = model->init_center_point - Eigen::Vector3f (center_point[0], center_point[1], center_point[2]);
                    }

                    model->performTracking(frameToFrameRGB, delta_translation, model_index,
                                           rgbOnly, icpWeight, pyramid, fastOdom, so3, maxDepthProcessed, textures[GPUTexture::RGB],
                            frame.timestamp, requiresFillIn(model));


                    std::ofstream ICPFile;
                    ICPFile.open(icp_error_file.c_str());
                    ICPFile << model->getFrameOdometry().lastICPError<<std::endl;
                    ICPFile.close();

                    std::ofstream all_ICPFile;
                    all_ICPFile.open(all_icp_error_file.c_str(),std::ofstream::out | std::ofstream::app);
                    all_ICPFile <<current_index<<","<< model->getFrameOdometry().lastICPError<<std::endl;
                    all_ICPFile.close();
                ...
                {
                    model->performTracking(frameToFrameRGB,  Eigen::Vector3f(0, 0, 0), model_index,
                                           rgbOnly, icpWeight, pyramid, fastOdom, so3, maxDepthProcessed, textures[GPUTexture::RGB],
                            frame.timestamp, requiresFillIn(model));
                    model_index++;
                }
        ...
        if (!rgbOnly && trackingOk && !lost)
        {
            ...
            float icp_error_thresh = icp_error_threshold_matlab;
            TICK("indexMap");
            unsigned int model_index = 0; // Don't interfere with the background
            for (auto model : models)
            {
                if((model->getFrameOdometry().lastICPError < icp_error_thresh) && (model_index > 0) && (std::isnan(model->getFrameOdometry().lastICPError) == false))
                {
